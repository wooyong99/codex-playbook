...
def check_agent_workflow_architecture(errors):
    script = ROOT / ".agents/scripts/validate-agent-workflow-architecture.py"
    ...
    if result.returncode != 0:
        ...
        errors.append(
            "agent workflow architecture validation failed"
            + (f": {details}" if details else "")
        )


def check_enterprise_eval_suite(errors):
    script = ROOT / ".agents/scripts/validate-enterprise-eval-suite.py"
    if not script.exists():
        errors.append(f"{script.relative_to(ROOT)}: missing")
        return
    result = subprocess.run(
    ...
    if result.returncode != 0:
        details = (result.stdout + result.stderr).strip()
        errors.append(
            "enterprise eval suite validation failed"
            + (f": {details}" if details else "")
        )


def check_skill_authoring_quality(errors):
    script = ROOT / ".agents/scripts/validate-skill-authoring-quality.py"
    if not script.exists():
        errors.append(f"{script.relative_to(ROOT)}: missing")
        return
    result = subprocess.run(
    ...
    if result.returncode != 0:
        details = (result.stdout + result.stderr).strip()
        errors.append(
            "skill authoring quality validation failed"
            + (f": {details}" if details else "")
        )
...
def main():
    ...
    check_agent_workflow_architecture(errors)
    check_enterprise_eval_suite(errors)
    check_skill_authoring_quality(errors)
