...
- `rule_id`: 이 계약 문서의 `Frontend Rule ID 형식`을 따른다. 아직 등록되지 않은 규칙은 `UNREGISTERED`로 둔다.
...
Frontend Architecture Reviewer의 `rule_id`는 frontend 규칙 공간 안에서만 안정적인 식별자로 사용한다. Rule ID는 규칙 원문을 대신하지 않고, reviewer 결과, 평가 기록, 수정 backlog가 같은 frontend 규칙을 가리키게 만드는 이름표다.

```text
FRONTEND-{UNIT}-{TOPIC}-{NNN}
```

| 파트 | 설명 | 예시 |
|------|------|------|
| `FRONTEND` | frontend reviewer 규칙 영역 | `FRONTEND` |
| `UNIT` | frontend 아키텍처 단위나 문서 단위 | `FSD`, `QUERY`, `ROUTING`, `COMPONENT`, `STATE`, `UX`, `PERFORMANCE` |
| `TOPIC` | 세부 주제 | `IMPORT`, `CACHE`, `BOUNDARY`, `A11Y`, `RENDER` |
| `NNN` | 3자리 일련번호 | `001`, `002` |

예시:

- `FRONTEND-FSD-IMPORT-001`
- `FRONTEND-QUERY-CACHE-001`
- `FRONTEND-ROUTING-BOUNDARY-001`

운영 원칙:

- 한 번 공개된 `rule_id`는 의미를 바꾸지 않는다. 의미가 바뀌면 새 ID를 만든다.
- 규칙 원문은 `source_path`가 가리키는 frontend Source of Truth 문서가 소유한다.
