# Implement Frontend Reference Map

이 디렉토리는 `implement-frontend` 스킬의 frontend 실행 지식을 계층별로 나눈다.
...
`implement-frontend`는 prompt 본문을 길게 넘기는 방식이 아니라 파일 기반 입출력으로 서브에이전트를 호출한다.
