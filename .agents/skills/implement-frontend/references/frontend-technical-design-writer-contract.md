...
에이전트 정의 파일(`.codex/agents/frontend-technical-design-writer.toml`)이 아닌 이 문서가 Agent D의 입출력 payload schema, 정상 결과 신호, 체크포인트 판단 기준, 체크포인트 파일 템플릿의 단일 출처다.
...
- Agent D의 역할 철학은 `.codex/agents/frontend-technical-design-writer.toml`이 제공한다.
...
input:
  ...
  source_of_truth:
    - path: <이번 설계에 적용할 frontend 기준 문서 또는 섹션>
      reason: <선별 이유>
artifacts:
...
output_contract:
  path: .agents/skills/implement-frontend/references/frontend-technical-design-writer-contract.md
  section: Output
```
