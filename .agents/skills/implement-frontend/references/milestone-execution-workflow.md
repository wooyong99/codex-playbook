...
각 frontend 마일스톤은 아래 순서로 실행한다.

1. `frontend-technical-design-writer`를 호출해 frontend TDD를 작성하거나 스킵 근거를 받는다.
2. `frontend-implementation-engineer`를 호출해 구현 결과를 받는다.
3. `frontend-architecture-reviewer`와 필요한 supplemental reviewer를 호출해 frontend 기준 준수 여부를 검토한다.
4. 위반이 있으면 같은 frontend A 인스턴스에 수정 작업을 맡기고 reviewer를 다시 호출한다.
5. 호출된 모든 reviewer가 `pass`를 반환하면 frontend 마일스톤을 완료한다.
...
B에게는 A 구현 결과 파일, D 설계 결과 파일, 이번 검토의 `[Source of Truth]`, B의 `[결과 파일]`, `[체크포인트 파일]`, `[출력 규격]`을 전달한다. 기준 문서와 TDD 결정은 B 계약의 Input 필드로 전달하며, agent TOML이 정적으로 소유하지 않는다.
