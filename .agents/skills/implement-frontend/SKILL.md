...
description: 프론트엔드 기능 구현, 리팩토링, UI 상태/API client/query/cache/rendering/UI-UX 변경을 기술설계(D) → frontend 구현(A) → frontend 아키텍처 검토(B) → 수정 루프로 실행하는 frontend 전용 스킬.
---
...
`implement-frontend`는 frontend 변경을 하나 이상의 마일스톤으로 나누고, 각 마일스톤을 설계(D) → 구현(A) → 아키텍처 검토(B) → 수정 루프로 실행하는 frontend 전용 오케스트레이션 스킬이다.
...
- Agent D: `frontend-technical-design-writer`
- Agent A: `frontend-implementation-engineer`
- Agent B: `frontend-architecture-reviewer`
