...
체크포인트 경로, 프롬프트 필드 이름, 출력 신호 포맷은 위 계약 문서가 단일 출처다. 이 문서에는 흐름과 의사결정만 유지하고, 문자열 예시는 가능한 한 계약 문서를 링크로 대체한다.
...
```bash
run_id=$(date +%Y%m%d-%H%M%S)-$RANDOM && mkdir -p ".agents/checkpoints/$run_id"
```
...
- `prompt`: [`references/design-writer-contract.md`](references/design-writer-contract.md) 의 Input 섹션 형식으로 구성한다. 현재 마일스톤의 제목·요구사항·실제 저장소 기준 `[프로젝트 컨텍스트]`·관련 레이어·도메인을 채운다.
...
```
체크포인트 파일을 Read한 뒤, [`references/design-writer-contract.md`](references/design-writer-contract.md) 의 체크포인트 재호출 규격대로 D를 재호출한다.
...
- `prompt`: [`references/code-writer-contract.md`](references/code-writer-contract.md) 의 Input > Case A 형식으로 구성한다. 현재 마일스톤의 제목·요구사항·실제 저장소 기준 `[프로젝트 컨텍스트]`·관련 레이어·도메인을 채우고, D 결과에 따라 `[기술설계문서]` 필드를 포함하거나 생략한다.
...
```
체크포인트 파일을 Read한 뒤, [`references/code-writer-contract.md`](references/code-writer-contract.md) 의 체크포인트 재호출 규격대로 A를 재호출한다.
...
- `prompt`: [`references/architecture-reviewer-contract.md`](references/architecture-reviewer-contract.md) 의 Input 섹션 형식으로 구성한다. A가 반환한 변경 파일 목록, 핵심 설계 결정, 그리고 D가 만든 `[기술설계문서]` 경로가 있으면 함께 채운다.
...
B의 응답 처리:
- **응답 첫 줄이 `CONTEXT_CHECKPOINT:` 신호인 경우**: 체크포인트 파일을 Read한 뒤 [`references/architecture-reviewer-contract.md`](references/architecture-reviewer-contract.md) 의 체크포인트 재호출 규격대로 B를 재호출한다. 완료된 파일의 위반 결과는 이전 응답에서 누적해서 보관.
...
- `prompt`: [`references/code-writer-contract.md`](references/code-writer-contract.md) 의 Input > Case B 형식으로 구성한다. B가 반환한 위반 YAML을 채운다. 마일스톤 내 A 인스턴스를 이어 사용하므로 Case A와 동일한 체크포인트 파일을 사용한다.
