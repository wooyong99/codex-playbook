...
[출력 규격]: 이 문서(.agents/skills/implement/references/frontend-technical-design-writer-contract.md) — Output 섹션 그대로.
```
...
체크포인트 판단은 상대 기준을 먼저 적용하고, 절대 수치는 안전장치로만 사용한다. 남은 작업이 없고 곧 `TDD_CREATED:` 또는 `TDD_SKIPPED:`를 반환할 수 있으면 `CONTEXT_CHECKPOINT:` 신호를 반환하지 말고 정상 완료한다. 단, 정상 완료 전에도 `[체크포인트 파일]`에는 완료 snapshot을 반드시 저장한다.
