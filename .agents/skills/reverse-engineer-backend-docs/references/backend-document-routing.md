...
이 문서는 분석 결과를 `docs/backend` 문서 구조로 라우팅하는 소유권 기준만 정의한다.

- 코드에서 어떤 신호를 찾을지는 [codebase-analysis-guide.md](codebase-analysis-guide.md)가 소유한다.
- 선택된 파일을 어떤 섹션으로 작성할지는 [backend-doc-templates.md](backend-doc-templates.md)가 소유한다.
- 이 문서는 문서 위치, 실행 모드 선택, 기존 문서 분류, 갱신 범위, 금지 규칙을 정의한다.
- 개별 문서 템플릿 본문이나 코드 샘플링 절차를 중복 소유하지 않는다.
