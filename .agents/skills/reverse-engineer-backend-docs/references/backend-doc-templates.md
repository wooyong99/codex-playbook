...
- 정책 문서는 전역 핵심 원칙과 금지 규칙을 소유한다. 실제 코드 구조와 반복 구현 방식은 `docs/backend/architecture` 하위 문서에서 정책을 링크해 설명한다.
```

## Policy Detail

정책 문서는 모든 아키텍처 단위가 따라야 하는 전역 핵심 원칙과 금지 규칙을 담는다.
...
- [ ] 전역 정책으로 유지해야 할 핵심 원칙과 금지 규칙만 담았다.
- [ ] 실제 코드 구조나 반복 구현 방식은 architecture 문서로 위임했다.
- [ ] 정책 판단의 근거가 되는 파일, 패키지, 클래스, 설정 경로를 남겼다.
- [ ] 새 정책 문서 추가·삭제·이름 변경이 `docs/backend/policies/README.md`에 반영되었다.
```
...
- [ ] 실제 코드 위치, 책임, 의존 경계가 코드 근거와 일치한다.
- [ ] 정책 원문을 중복 서술하지 않고 관련 정책 링크로 연결했다.
- [ ] 세부 구현 방식은 이 문서가 아니라 `strategies/README.md` 또는 strategy detail로 위임했다.
- [ ] 단위 문서 추가·삭제·이름 변경이 `docs/backend/architecture/README.md`에 반영되었다.
```
...
- [ ] strategy 문서 추가·삭제·이름 변경이 가장 가까운 `strategies/README.md`에 반영되었다.
