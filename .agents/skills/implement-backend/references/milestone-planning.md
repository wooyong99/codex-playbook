...
- `입력 근거`: 상위 input artifact에 포함된 요구사항·정책·계약 근거. 없으면 "없음"
- `계약 근거`: backend가 구현하거나 제공해야 할 API/data 계약. 없으면 "없음"
...
- 명시적 제외사항은 무엇인가
- 상위 input artifact의 요구사항·정책·상태·도메인 용어 중 backend 책임은 무엇인가
- 상위 input artifact의 계약 근거 중 backend가 구현하거나 제공해야 할 API/data 계약은 무엇인가
