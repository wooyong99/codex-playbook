...
입력·출력 원칙:
정체성 기준 체크리스트:
- [ ] 이 역할의 책임과 판단 기준이 정체성만으로 설명된다.
- [ ] 특정 스킬, 계약 파일, 고정 Source of Truth 경로 없이도 독립적으로 동작한다.
- [ ] 구현자, reviewer, writer 등 다른 역할의 책임을 대신 수행하지 않는다.
- [ ] 입력이 바뀌어도 고정 절차가 아니라 역할 판단 기준으로 결론을 낸다.
금지 규칙:
```
...
- `정체성 기준 체크리스트`는 호출 workflow가 아니라 서브에이전트의 독립 역할 정체성을 기준으로 작성한다.
