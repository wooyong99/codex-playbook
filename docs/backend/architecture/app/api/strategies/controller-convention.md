...
- 성공 응답은 [support/api response envelope](../../../support/api/strategies/response-envelope-convention.md)의 `BaseResponse.success(...)`를 따른다.
- 생성 성공은 `201 Created`, 일반 조회와 변경 성공은 `200 OK`, body 없는 삭제는 프로젝트 정책에 따라 `204 No Content` 또는 envelope 응답 중 하나로 통일한다.
- 오류 응답은 Controller에서 만들지 않고 [support/api exception response](../../../support/api/strategies/exception-response-convention.md)을 따른다.
