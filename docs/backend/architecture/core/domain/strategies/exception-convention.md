...
- HTTP 상태 코드 매핑을 `support/api` 책임으로 분리한다.
...
HTTP 응답 변환은 [support/api exception response convention](../../../support/api/strategies/exception-response-convention.md)이 소유한다.

## 책임

- 도메인 실패를 domain language 기반 code와 message로 표현한다.
- framework-independent error type으로 `support/api` 매핑에 필요한 의미만 전달한다.
