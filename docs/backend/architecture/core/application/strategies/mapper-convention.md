...
HTTP Request/Response DTO 변환은 `app/api`가 소유한다. 외부 API DTO 번역은 `external/integration` adapter가 소유한다.
...
- Mapper가 `app/api` Response DTO, 외부 API DTO, persistence Entity 변환을 application 내부 변환처럼 소유하지 않는다.
