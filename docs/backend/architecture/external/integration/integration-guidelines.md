...
이 문서는 `backend/external/integration` 모듈의 실제 코드 위치, 책임, 의존 경계, 구현 전략을 정리한다.

## 코드 위치

- `backend/external/integration` - 외부 시스템 Port 구현, ApiClient, DTO, 예외 번역, Mock Adapter 후보를 담당한다.
...
- depends on: `core/application`, `core/domain`, external systems
...
- [ ] 로컬 개발 환경에서 실제 외부 시스템 없이 성공·실패 주요 흐름이 재현된다.
