...
이 문서는 `support/api`에서 예외를 HTTP status와 공통 오류 응답으로 변환하는 전략을 정리한다.
...
Domain 예외 계층과 `{Domain}ErrorCode`는 [domain exception convention](../../../core/domain/strategies/exception-convention.md)이 소유한다.
...
`CoreErrorType`은 HTTP 의미를 추상화한 domain 타입이고, 실제 `HttpStatus` 변환은 `support/api`에서 수행한다.
