...
다만 기본 지향점은 `Service`와 `Handler`에 Port 참조를 모으는 것이다.

- Service: aggregate command 처리에 필요한 조회와 저장
- Handler: 여러 개념 영역에서 재사용되는 외부 자원 접근 또는 경계 보호
...
- 같은 Port 호출이 여러 Service에서 반복되거나 다른 개념 영역을 보호해야 하면 Handler로 내린다.
...
- Port 참조는 가능하면 Service와 Handler에 모여 있다.
