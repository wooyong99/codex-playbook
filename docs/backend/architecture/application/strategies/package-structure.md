...
app, domain, storage, external 단위의 파일 구조는 각 단위의 전략 문서가 소유한다.
...
- app 계층의 HTTP Request/Response DTO를 application `dto/`에 두지 않는다.
...
- app, storage, external 구현체나 DTO를 application 패키지 안으로 끌어오지 않는다.
