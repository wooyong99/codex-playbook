...
HTTP Request/Response DTO는 app 계층이 소유한다. 외부 API DTO는 external adapter가 소유한다. JPA Entity와 QueryDsl 타입은 storage adapter가 소유한다.
...
- Service, Validator, Strategy가 UseCase `Result` 또는 app 계층 Response DTO를 반환하지 않는다.
