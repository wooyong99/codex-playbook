...
- Markdown 문서, 스킬, 서브에이전트 정의는 목적, 범위, 책임, 전체 흐름을 먼저 드러낸다.
- 구체적인 처리 방법, 구현 절차, schema, 코드 스타일은 하위 섹션 또는 별도 reference 문서로 분리한다.
- 문서 내용은 상위 개념에서 세부 구현 순서로 읽히게 작성한다.
- 스킬을 생성하거나 크게 재구성할 때는 `write-skill-artifact` 스킬을 사용한다.
- 서브에이전트 TOML을 생성하거나 크게 재구성할 때는 `write-subagent-artifact` 스킬을 사용한다.
- 일반 Markdown 문서 작성은 해당 문서 영역의 README, 전용 스킬, 또는 문서 소유권 규칙을 따른다.
